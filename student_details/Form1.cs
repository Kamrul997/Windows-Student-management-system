...
namespace student_details
{
    public partial class Student_details : Form
    {
        ...
        public int rowId;
        SqlConnection con = new SqlConnection("Data Source=DESKTOP-FS8H5V0; Initial Catalog=STUDENT; TrustServerCertificate=True; Integrated Security=True ");
        ...
        private void update_Click(object sender, EventArgs e)
        {
            ...
            String NameError = NameValidate(StudentName.Text);
            String AgeError = AgeValidate(Age.Text);
            String EmailError = EmailUpdateValidate(Email.Text);
            
            String MobileError = MobileUpdateValidate(Mobile.Text);
            ...
            {
                string sqlQuery = "update StudentInfo set Name = " + "'" + StudentName.Text + "' , Age = " + "'" + Convert.ToInt32(Age.Text) + "' , MobileNumber = " + "'" + Mobile.Text + "' , Email = " + "'" + Email.Text + "' , Gender = " + "'" + Gender + "' , Class = " + "'" + StudentClass.SelectedItem + "' , Ad_date = " + "'" + Date.Value.ToShortDateString() + "'   where ID = " + tID;
        ...
        public string EmailValidate(string StudenEmail)
        {
            ...
            else if (StudenEmail.Length > 50)
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    var sname = row.Cells[4].Value.ToString();

                    if (sname==StudenEmail)
                    {
                        EmailErrorMessage = "Your Email is Already Exist enter a new Email";
                    }
                }               
            
            return EmailErrorMessage;
        }
        public string EmailUpdateValidate(string StudenEmail)
        {
            string EmailErrorMessage = string.Empty;
            string email = Email.Text;
            System.Text.RegularExpressions.Regex expr = new System.Text.RegularExpressions.Regex(@"^[a-zA-Z][\w\.-]{2,28}[a-zA-Z0-9]@[a-zA-Z0-9][\w\.-]*[a-zA-Z0-9]\.[a-zA-Z][a-zA-Z\.]*[a-zA-Z]$");
        ...
        public string MobileValidate(string StudenMobile)
        {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                var sname = row.Cells[3].Value.ToString();

                if (sname == StudenMobile)
                {
                    MobileErrorMessage = "Your Mobile Number is Already Exist enter a new Email";
                }
            }
            return MobileErrorMessage;
        }
        public string MobileUpdateValidate(string StudenMobile)
        {
            string MobileErrorMessage = string.Empty;
            if (StudenMobile.Length > 20)
            {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                ...
                if (sname == StudenMobile)
                {
                    ...
                    if (count > 1)
                    {
                        MobileErrorMessage = "Your Mobile Number is Already Exist enter a new Mobile Number";
                    }
                    else 
                    {
                        MobileErrorMessage = "";
